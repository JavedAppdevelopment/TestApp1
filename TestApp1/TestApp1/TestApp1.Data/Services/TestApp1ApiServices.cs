...
using TestApp1.Common.Exceptions;
using TestApp1.Common.Interfaces;
using TestApp1.Common.Logging;
using TestApp1.Data.Interfaces;

namespace TestApp1.Data.Services
{
	public class TestApp1ApiServices : ITestApp1ApiServices
	{
		...
		public TestApp1ApiServices(IAppConfig appConfig, ILogger logger, IConnectivityHelper connectivityHelper)
		{
		...
		private async Task<T> ExecuteRequest<T>(string api, object data, Uri url, HttpClient client, HttpRequestMessage request)
		{
			if (!_connectivityHelper.IsConnected)
				throw new TestApp1ApiExceptions(ApiErrorCodes.ConnectivityLost, "No connectivity found when about to make an api request");
			...
			{
				throw TestApp1ApiExceptions.ProcessApiException(ex, id, api, request.Method.ToString(), url.ToString(), data);
			}
			...
			if (!response.IsSuccessStatusCode)
			{
				...
				throw new TestApp1ApiExceptions(ApiErrorCodes.GenericError, response.ReasonPhrase) { ResponseContent = responseContent };
			}
